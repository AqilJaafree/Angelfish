...
import { formatBadges, formatBoard, formatFeeTier, formatRsi, formatUsd, formatUsd18 } from './format';
...
describe('formatFeeTier', () => {
  // PancakeSwap's ladder, not Uniswap's: 0.25% where Uniswap has 0.3%.
  it('renders the four PancakeSwap v3 tiers', () => {
...
describe('formatBoard', () => {
  it('renders three lines per row, ending with the contract address', () => {
    const out = formatBoard({
      rows: [{ ...base, feeTier: '0.25%', marketCapUsd: 1_200_000, verified: true, risk: 'clean' }],
      block: 200,
      fromBlock: 195,
      variant: 'main',
      label: 'PancakeSwap v3 (BNB)',
    });
    const lines = out.split('\n');
    expect(lines[0]).toContain('PancakeSwap v3 (BNB)');
    expect(lines[0]).toContain('blocks 195–200');
    expect(lines[1]).toBe('🥇 TEST ✅🟢 (0.25%) (warming) · RSI — · MC $1.2M');
