...
// Fee rates are hundredths of a bip: 2500 -> "0.25%".
export function formatFeeTier(fee: number): string {
  const pct = fee / 10_000;
  return `${Number(pct.toFixed(4))}%`;
}

// A USD amount held as a bigint in 1e18 fixed point -> "$1.2M".
//
// Replaces the Ethereum build's formatEth: pools here do not share one anchor, so
// there is no single currency symbol a volume could carry (see bsc/anchors.ts).
// `undefined` — a BNB-quoted pool during a failed BNB/USD read — renders as a dash,
// never as $0, so an unpriced row cannot be misread as an inactive one.
export function formatUsd18(value: bigint | undefined): string {
  if (value == null) return '—';
  const usd = Number(value) / 1e18;
  if (usd === 0) return '$0';
...
export function formatRow(row: MoversRow, index: number): string {
  ...
  const line2 = `    ${formatUsd18(row.volumeUsd)} vol · ${row.swaps} swaps · ${row.traders} traders · ${formatUsd18(row.feesUsd)} fees`;
