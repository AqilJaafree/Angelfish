...
import { WBNB, USDT, V3_FACTORY, SEL_TOKEN0, SEL_TOKEN1, SEL_FEE, SEL_GET_POOL } from '../config';
...
function makeCaller(opts: {
  ...
  const call = async (to: string, data: string): Promise<string> => {
    ...
    if (data === SEL_TOKEN0) return word(opts.token0 ?? WBNB);
    if (data === SEL_TOKEN1) return word(opts.token1 ?? TOKEN);
    if (data === SEL_FEE) return word((opts.fee ?? 2500).toString(16));
...
describe('resolvePoolMeta', () => {
  it('resolves a canonical WBNB pool and records the anchor side', async () => {
    const { call } = makeCaller({ token0: WBNB, token1: TOKEN });
  ...
  it('handles the anchor as token1', async () => {
    const { call } = makeCaller({ token0: TOKEN, token1: WBNB });
    const meta = await resolvePoolMeta(POOL, {}, call);
    expect(meta?.anchorIsToken0).toBe(false);
    expect(meta?.token).toBe(TOKEN);
  });

  // BSC needs more than one anchor — a stable-quoted pool is the majority case here
  // and would be invisible to a WBNB-only check.
  it('resolves a USDT-anchored pool as kind usd', async () => {
    const { call } = makeCaller({ token0: USDT, token1: TOKEN });
    const meta = await resolvePoolMeta(POOL, {}, call);
    expect(meta?.anchorKind).toBe('usd');
    expect(meta?.anchor).toBe(USDT);
  });

  // Both sides anchored = no subject token. WBNB/USDT is among the busiest pools on
  // the chain, so this branch fires constantly and must drop the pool.
  it('rejects an anchor-on-both-sides pair without asking the factory', async () => {
    const { call, calls } = makeCaller({ token0: USDT, token1: WBNB });
    const cache: Record<string, PoolMeta | null> = {};
  ...
  // A v3 FORK answers token0/token1/fee exactly like the real thing, so only the
  // factory can tell them apart.
  ...
  it('caches the negative for a confirmed unanchored pool without asking the factory', async () => {
