...
// A 0x-prefixed bytes32 as a 32-byte ABI argument word — i.e. itself, unpadded.
//
// Exists as a named function rather than an inline `.slice(2)` because the Ethereum
// build needed the opposite: Uniswap v4's PositionManager keys its PoolKey mapping
// on a TRUNCATED bytes25, so a PoolId had to be left-aligned with its low 7 bytes
// zeroed, and right-aligning it read the wrong slot and returned an empty key —
// indistinguishable from "unknown pool". PancakeSwap Infinity keys
// CLPoolManager.poolIdToPoolKey on the FULL bytes32, so no truncation applies and
// the id goes through as-is.
export function bytes32Arg(bytes32: string): string {
  return bytes32.slice(2).padStart(64, '0');
}
