...
describe('resolveToken', () => {
  it('resolves a known symbol case-insensitively', () => {
    expect(resolveToken('usdc')?.address).toBe('0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d');
    // 18 on BSC, not the 6 it is on Ethereum — toBaseUnits gates on this.
    expect(resolveToken('USDC')?.decimals).toBe(18);
  });
  it('accepts a raw address and marks decimals unknown', () => {
    const t = resolveToken('0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d');
    expect(t?.address).toBe('0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d');
...
describe('sortTokens', () => {
  ...
  it('orders by address and reports the flip', () => {
    ...
    const wbnb = resolveToken('WBNB')!;
    const { token0, token1, flipped } = sortTokens(wbnb, usdc);
    expect(token0.symbol).toBe('USDC'); // 0x8ac7… < 0xbb4c…
    expect(token1.symbol).toBe('WBNB');
    expect(flipped).toBe(true);
    expect(sortTokens(usdc, wbnb).flipped).toBe(false);
  });
});
...
describe('rangeTicks', () => {
  ...
  it('aligns both bounds to the tier spacing', () => {
    for (const [fee, spacing] of [[500, 10], [2500, 50], [10000, 200]] as const) {
...
describe('singleSidedTicks', () => {
  ...
  // Pinned to a position this code actually opened. It executed on Ethereum against
  // Uniswap v3, before the BNB Chain port — the tick maths is protocol-identical, so
  // the regression still holds and is left on the real numbers rather than invented ones.
  it('reproduces the executed live position', () => {
  ...
  it('aligns both bounds to the tier spacing', () => {
    for (const [fee, spacing] of [[100, 1], [500, 10], [2500, 50], [10000, 200]] as const) {
...
describe('knownToken', () => {
  it('maps a known address back to its symbol and decimals', () => {
    expect(knownToken('0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d')).toEqual({
...
describe('positionAmounts', () => {
  ...
  // Same live position as above — see the note on rangeTicks.
  it('reproduces the deposit of the executed live position', () => {
...
describe('priceFromSqrt', () => {
  // A 6-vs-18 decimal pair, which BSC does not have (its stables are all 18) — kept
  // precisely because it exercises the decimal correction that a same-decimals pair
  // would leave untested.
  it('prices a 6/18-decimal pool in a sane range', () => {
...
describe('buildMintParams', () => {
  ...
  it('emits an OBJECT tuple, which is what KeeperHub can encode', () => {
    const p = buildMintParams({
      token0: usdc, token1: wbnb, fee: 500, tickLower: 100, tickUpper: 200,
  ...
  it('carries amounts as decimal strings, never numbers', () => {
    const p = buildMintParams({
      token0: usdc, token1: wbnb, fee: 500, tickLower: 0, tickUpper: 10,
