...
// v3's absolute tick bounds, identical in PancakeSwap and Uniswap — they fall out
// of the 1.0001^tick price formula, not from any protocol constant. A range outside
// these reverts.
...
// would produce a wildly wrong band at any tick far from zero.
...
//
// Approximated in floating point rather than porting the protocol's TickMath, whose
...
// WBNB's payable deposit(): the on-ramp from raw BNB, which is not an ERC20 and so
// cannot be deposited into a pool directly. WBNB exposes the same deposit()/
// withdraw() pair as WETH, so only the name and the token address change.
export const WBNB_DEPOSIT_ABI = JSON.stringify([
