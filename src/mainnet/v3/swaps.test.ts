...
import { aggregateSwaps, feesToWeth, rankTopN, PoolMeta, PoolAggregate } from './swaps';
import { DecodedSwap } from '../decode';

const POOL = '0xpool';
const TOKEN = '0xtoken';
const meta = (wethIsToken0: boolean, fee = 3000): Map<string, PoolMeta> =>
  new Map([[POOL, { pool: POOL, token: TOKEN, fee, wethIsToken0 }]]);
...
describe('aggregateSwaps', () => {
  it('sums the absolute WETH leg regardless of direction', () => {
    const agg = aggregateSwaps(
      [
        { pool: POOL, swap: swap(100n, -50n) }, // WETH in
        { pool: POOL, swap: swap(-30n, 20n) }, // WETH out
      ],
      meta(true)
    );
    expect(agg.get(POOL)!.volumeWeth).toBe(130n);
  ...
  it('takes the fee from the positive (input) side only', () => {
    ...
    expect(agg.get(POOL)!.fees0).toBe(3000n); // 1e6 * 3000/1e6
    expect(agg.get(POOL)!.fees1).toBe(0n);
  });

  it('drops swaps from pools with no metadata (forks / non-WETH)', () => {
  ...
  it('reads the WETH leg from amount1 when WETH is token1', () => {
    const agg = aggregateSwaps([{ pool: POOL, swap: swap(999n, -7n) }], meta(false));
    expect(agg.get(POOL)!.volumeWeth).toBe(7n);
  });
});
...
describe('feesToWeth', () => {
  ...
  it('is a plain sum when the pool price is 1', () => {
    expect(feesToWeth(10n, 20n, Q96, true)).toBe(30n);
    expect(feesToWeth(10n, 20n, Q96, false)).toBe(30n);
  });

  it('does not divide by zero on an uninitialized price', () => {
    expect(feesToWeth(10n, 20n, 0n, true)).toBe(10n);
  });

  it('converts the token side at the pool price', () => {
    const sqrt = Q96 * 2n; // price = 4 (token1 per token0)
    // WETH = token0: 20 token1 is worth 20/4 = 5 WETH.
    expect(feesToWeth(10n, 20n, sqrt, true)).toBe(15n);
    // WETH = token1: 10 token0 is worth 10*4 = 40 WETH.
    expect(feesToWeth(10n, 20n, sqrt, false)).toBe(60n);
  });
});
