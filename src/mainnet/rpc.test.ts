...
describe('isSplittableError', () => {
  it('matches the range-cap message free mainnet endpoints actually return', () => {
    expect(
      isSplittableError('You can make eth_getLogs requests with up to a 10 block range')
    ).toBe(true);
  });
