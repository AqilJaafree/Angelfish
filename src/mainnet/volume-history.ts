...
export interface VolumeBucket {
  volume: string; // bigint wei does not survive JSON
  blocks: number;
}
...
export const MAX_HISTORY_BUCKETS = 24; // ~2h at 25 blocks/bucket
...
export const MIN_SPIKE_VOLUME_WEI = BigInt(
  process.env.SPIKE_MIN_VOLUME_WEI ?? '50000000000000000' // 0.05 ETH
);
...
export function spikeScore(
  ...
  if (windowVolume < MIN_SPIKE_VOLUME_WEI) return undefined;
