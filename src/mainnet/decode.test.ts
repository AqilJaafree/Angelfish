...
describe('toBytes25Arg', () => {
  it('LEFT-aligns the id: keeps the high 25 bytes, zeroes the low 7', () => {
    const poolId = '0x' + 'ab'.repeat(32);
    const arg = toBytes25Arg(poolId);
    expect(arg).toHaveLength(64);
    expect(arg.slice(0, 50)).toBe('ab'.repeat(25));
    expect(arg.slice(50)).toBe('0'.repeat(14));
  });

  it('differs from right-alignment — the bug it exists to prevent', () => {
    const poolId = '0x' + '11'.repeat(32);
    expect(toBytes25Arg(poolId)).not.toBe(poolId.slice(2).slice(0, 50).padStart(64, '0'));
  });
});
...
describe('decodeSymbol', () => {
  it('decodes an ABI-encoded string', () => {
    const text = Buffer.from('WETH', 'utf8').toString('hex');
    const result = '0x' + word('20') + word('4') + text.padEnd(64, '0');
    expect(decodeSymbol(result)).toBe('WETH');
  });
